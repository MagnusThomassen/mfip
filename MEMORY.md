...
*Last updated: 2026-05-19 — Session 19 PR #64: BETA + EE scalar extraction shipped. `_read_scalar` helper + `_NA_VALUES` taxonomy (broad `#`-prefix guard for unknown Excel error strings) + per-sheet `na_cells` populated with `SHEET!COORD` provenance + two-tier fixture strategy (template for shape, committed `tests/fixtures/bloomberg/EQNR_NO_2026-05-08.xlsx` for values). 79 → 102 tests green. DVD / ANR / HP_Monthly / HP_Daily / RV_Comps still stubbed pending PRs #65-#66.*
