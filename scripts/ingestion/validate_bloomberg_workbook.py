...
Usage from venv:
    ...
    python scripts\\ingestion\\validate_bloomberg_workbook.py --file C:\\MFIP\\bloomberg_archive\\EQNR_NO\\2026-05-08\\EQNR_NO_2026-05-08.xlsx
"""
...
def validate_all(archive_root: Path) -> list[ValidationReport]:
    ...
    for prefix in UNIVERSE:
        ...
        if not company_dir.is_dir():
            ...
            reports.append(r)
            continue
        latest = find_latest_workbook(company_dir)
        if latest is None:
            r = ValidationReport(file=company_dir)
            r.add(
